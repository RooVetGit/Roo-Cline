...
describe('SearchReplaceMultilineDiffStrategy', () => {
...
>>>>>>> REPLACE`
            ...
            // Even though we target lines 5-7, it should find the match at lines 9-11
...
>>>>>>> REPLACE`
            ...
            // Targeting lines 5-7, function five() is more than 5 lines away
            const result = strategy.applyDiff(originalContent, diffContent)
            expect(result.success).toBe(false)
        })
    })
...
>>>>>>> REPLACE`
            ...
            expect(result.success).toBe(false)
            if (!result.success) {
                expect(result.error).toContain('Overlapping search blocks detected')
            }
        })

        it('should handle adjacent search blocks', () => {
            const originalContent = `
function validate() {
    if (!data) {
        return false;
...
>>>>>>> REPLACE`
            ...
            expect(result.success).toBe(true)
            if (result.success) {
                expect(result.content).toBe(`function validate() {
    if (!data) {
        throw new Error('Missing data');
    }
    if (!schema) {
